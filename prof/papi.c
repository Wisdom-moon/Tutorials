...
if (PAPI_event_name_to_code("L2_CODE_READ_MISS_CACHE_FILL", &native) != PAPI_OK)
{
    printf("Error: PAPI_event_name_to_code\n");
    exit(1);
}
if (PAPI_add_event(EventSet, native) != PAPI_OK)
{
    printf("Error: PAPI_add_named_event0\n");
    exit(1);
}
if (PAPI_add_named_event(EventSet, "PAPI_L1_ICM") != PAPI_OK)
{
    printf("Error: PAPI_add_named_event1\n");
    exit(1);
}
if (PAPI_add_named_event(EventSet, "PAPI_L1_DCM") != PAPI_OK)
{
    printf("Error: PAPI_add_named_event2\n");
    exit(1);
}
...
printf("Thread %d : L1 ICM %lld, L1 DCM %lld\n", omp_get_thread_num(), values[0], values[1]);
}
