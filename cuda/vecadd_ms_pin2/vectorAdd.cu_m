...
int main(int argc, char** argv)
{
    ...
    h_A = (float *) mmap( NULL, size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANON, -1, 0 );
    if (h_A == 0) CleanupResources();
    cudaHostRegister(h_A, size, cudaHostRegisterMapped);

    h_B = (float *) mmap( NULL, size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANON, -1, 0 );
    if (h_B == 0) CleanupResources();
    cudaHostRegister(h_B, size, cudaHostRegisterMapped);

    h_C = (float *) mmap( NULL, size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANON, -1, 0 );
    if (h_C == 0) CleanupResources();
    cudaHostRegister(h_C, size, cudaHostRegisterMapped);
